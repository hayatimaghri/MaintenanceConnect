...
<aside class="hidden w-64 shrink-0 flex-col bg-[#0b1f3a] text-white lg:flex">
    ...
    <div class="flex-1 px-4 py-7">
        ...
        <nav class="mt-4 space-y-1">
            ...
            <a href="{{ route('competences.index') }}" class="flex items-center gap-3 rounded-lg {{ request()->routeIs('competences.*') ? 'bg-sky-500/15 text-sky-300' : 'text-slate-300 transition hover:bg-white/5 hover:text-white' }} px-3 py-3 text-sm font-semibold">
                <span class="flex h-7 w-7 items-center justify-center rounded-md {{ request()->routeIs('competences.*') ? 'bg-sky-500' : 'bg-white/10' }} text-xs font-black text-white">C</span>
                Compétences
            </a>
            <a href="{{ route('experiences.index') }}" class="flex items-center gap-3 rounded-lg {{ request()->routeIs('experiences.*') ? 'bg-sky-500/15 text-sky-300' : 'text-slate-300 transition hover:bg-white/5 hover:text-white' }} px-3 py-3 text-sm font-semibold">
                <span class="flex h-7 w-7 items-center justify-center rounded-md {{ request()->routeIs('experiences.*') ? 'bg-sky-500' : 'bg-white/10' }} text-xs font-black">E</span>
                Expériences
            </a>
            <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 rounded-lg {{ request()->routeIs('profile.*') ? 'bg-sky-500/15 text-sky-300' : 'text-slate-300 transition hover:bg-white/5 hover:text-white' }} px-3 py-3 text-sm font-semibold">
                <span class="flex h-7 w-7 items-center justify-center rounded-md {{ request()->routeIs('profile.*') ? 'bg-sky-500' : 'bg-white/10' }} text-xs font-black text-white">P</span>
                Mon profil
            </a>
        </nav>
        ...
        <div class="mt-4 space-y-2 px-3 text-xs leading-5 text-slate-400">
            <p>Les informations affichées dépendent de votre espace.</p>
        </div>
    </div>
    ...
    <div class="border-t border-white/10 p-4">
        <div class="mb-3 flex items-center gap-3 px-2">
            ...
            <div class="min-w-0">
                ...
                <p class="text-xs text-slate-400">{{ $sidebarUser->role }}</p>
            </div>
        </div>
